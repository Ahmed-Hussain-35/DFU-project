"""
DFU Analysis — FastAPI inference service (auto-detects .pth or .onnx)
Set DFU_MODEL to either:
  best_model.pth        -> uses PyTorch  (needs torch + segmentation-models-pytorch)
  dfu_model_fp16.onnx   -> uses onnxruntime (light)
Everything downstream (features -> infection -> wagner) is identical either way.
"""
...
# --- CONFIG ---
MODEL_PATH = os.getenv("DFU_MODEL", "dfu_model_fp16.onnx")   # or "best_model.pth"
IMG_SIZE   = 384
THRESHOLD  = 0.45
MIN_AREA   = 100
...
app = FastAPI(title="DFU Analysis Service", version="1.1")
app.add_middleware(CORSMiddleware, allow_origins=["*"],
                   allow_methods=["*"], allow_headers=["*"])

# --- BACKEND: .pth -> torch, .onnx -> onnxruntime ---
...
def analyze_features(image, mask):
    ...
    # looser yellow/slough range (merged from infection notebook) -> catches more pus
...
def compute_infection(features, coverage):
    # Guard: if almost no ulcer was segmented, the features are computed on a
    # handful of noisy pixels -> don't trust them. Report minimal and stop.
    if coverage < 1.0:
        # zero the features too so the UI doesn't show misleading noise bars
        for k in features:
            features[k] = 0.0
    ...
    # Rebalanced weights: necrosis no longer dominates (was 0.30) so a large raw RED
    # wound with no black tissue can still score appropriately. Sums to 1.00.
    score = (min(r * 2.5, 100) * 0.18 + min(n * 2.5, 100) * 0.22 + min(p * 4.0, 100) * 0.18 +
             min(s * 2.5, 100) * 0.15 + min(i * 1.8, 100) * 0.12 + sz * 0.15)
    # Each rule sets a "floor" the score can't drop below; keep the HIGHEST floor,
    # and only show a note if that floor actually beat the computed score.
    floor, reason = 0, None
    ...
    else:            level, color = "SEVERE INFECTION", "#c0392b"
    return {"score": round(score, 2), "level": level, "color": color, "override": override}


def compute_wagner(features, mask, shape, inf_score):
    n, p = features["Necrosis"], features["Pus"]
    coverage = (mask.sum() / (shape[0] * shape[1])) * 100
    if mask.sum() == 0:                return {"grade": 0, "title": "Grade 0 \u2014 Pre-ulcer / At Risk", "color": "#27ae60"}
    # tiny mask -> features unreliable, cap at superficial rather than misgrading
    if coverage < 1.0:                 return {"grade": 1, "title": "Grade 1 \u2014 Superficial / Minimal", "color": "#f1c40f"}
    if n > 50 and coverage > 15:       return {"grade": 5, "title": "Grade 5 \u2014 Extensive Gangrene", "color": "#7b0000"}
    if n > 30:                         return {"grade": 4, "title": "Grade 4 \u2014 Localized Gangrene", "color": "#c0392b"}
    if p > 15 or inf_score > 65:       return {"grade": 3, "title": "Grade 3 \u2014 Deep Ulcer + Abscess", "color": "#e74c3c"}
    if coverage > 5 or inf_score > 40: return {"grade": 2, "title": "Grade 2 \u2014 Deep Ulcer", "color": "#e67e22"}
    if coverage > 0:                   return {"grade": 1, "title": "Grade 1 \u2014 Superficial Ulcer", "color": "#f1c40f"}
    return {"grade": 0, "title": "Grade 0 \u2014 Pre-ulcer", "color": "#27ae60"}
...
@app.get("/health")
def health():
    return {"status": "ok", "backend": _BACKEND, "model": os.path.basename(MODEL_PATH)}
...
async def analyze(file: UploadFile = File(...)):
    ...
    wagner = compute_wagner(feats, mask, image.shape, infection["score"])
